...
export default function ShowSession({ session }: { session: Session }){
	...
	return (
      ...
      <div className="mt-6 flex justify-center gap-4">
        <Link href={`/plans/${session.plan_id}/cicles/${session.cicle_id}`} className="flex h-10 items-center rounded-lg bg-gray-100 px-4 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-200">
          Volver
        </Link>
      </div>
  </div>
